...
const EditProductPage = () => {
  ...
  const { product, loading: initialLoading, error: initialError } = useProduct(id);
  ...
  const handleSubmit = async e => {
    ...
    try {
      const { data } = await productApi.update(id, {
    ...
    } catch (err) { setApiError(err.response?.data?.message || 'Failed to update product.'); }
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="animate-pulse space-y-6">
          <div className="h-10 bg-surface-2 rounded-xl w-1/4"></div>
  ...
  if (initialError || !product) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 text-center">
        <p className="text-zinc-600 mb-4">{initialError || 'Product not found.'}</p>
        <Link to="/seller/products" className="text-violet-400 hover:text-violet-300">← Back to My Products</Link>
      </div>
    );
  }
