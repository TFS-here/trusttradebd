...
const AdminUsers = () => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {/* Filters */}
      <div className="card rounded-2xl p-4 flex flex-wrap gap-3">
        ...
        <select value={filters.role} onChange={e => setFilters(f => ({ ...f, role: e.target.value, page: 1 }))}
          className="bg-surface-3 border border-white/8 text-zinc-400 text-sm rounded-xl px-3 py-2.5 focus:outline-none focus:border-violet-500/40">
          ...
          <div className="divide-y divide-white/5">
            {users.map(user => (
              <motion.div key={user._id} layout
                className={`flex items-center gap-4 px-5 py-4 hover:bg-white/3 transition ${user.isBlocked ? 'opacity-50' : ''}`}>
                ...
                <AnimatePresence>
                  {feedback[user._id] && (
                    <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      className="text-xs text-emerald-400 font-medium shrink-0">✓ {feedback[user._id]}</motion.span>
                  )}
                </AnimatePresence>
